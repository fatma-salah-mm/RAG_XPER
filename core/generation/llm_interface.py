"""
core/generation/llm_interface.py for RAG_XPER

Backend-ready LLM interface supporting Google Gemini and local Ollama.
Includes automatic sub-batching, rate-limit backoff, and modern SDK support.
"""
...
from core.exceptions import LLMGenerationError, LLMTimeoutError
        ...
        for attempt in range(1, self._max_retries + 2):
            ...
            try:
                ...
                logger.warning(
                    ...
                    self._timeout, attempt, self._max_retries + 1,
                )
                raise LLMTimeoutError(
                    f"Gemini generation exceeded {self._timeout}s timeout"
                ) from exc
                ...
                if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "Quota exceeded" in err_str:
                    ...
                    logger.warning(
                        "Gemini rate limit (429) hit during generate. Sleeping %.1fs before retrying (%d/%d)...",
                        delay, attempt, self._max_retries + 1,
                    )
                    print(
                        f"⏳ تم الوصول لحد الطلبات (Rate Limit)، جاري الانتظار {int(delay)} ثانية ثم إعادة المحاولة تلقائياً...",
                        flush=True,
                    )
                    time.sleep(delay)
                else:
                    logger.warning(
                        "Gemini call failed on attempt %d/%d: %s", attempt, self._max_retries + 1, exc
                    )
                    time.sleep(1.0)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "Quota exceeded" in err_str:
                    ...
                    logger.warning(
                        "Gemini rate limit (429) hit on embed (attempt %d/%d). Sleeping %.1fs before retry...",
                        attempt, max_retries, delay
                    )
                    print(
                        f"⏳ تم الوصول لحد الطلبات المجاني لـ Gemini (Rate Limit). جاري الانتظار {int(delay)} ثانية ثم المتابعة تلقائياً ({attempt}/{max_retries})...",
                        flush=True,
                    )
                    time.sleep(delay)
                else:
                    logger.warning(
                        "Gemini embedding call failed on attempt %d/%d: %s", attempt, max_retries, exc
                    )
...
class OllamaLLM(BaseLLM):
    """Fully local implementation of ``BaseLLM`` using Ollama."""
    ...
    def __init__(
        ...
        self._executor = concurrent.futures.ThreadPoolExecutor(max_workers=4)
        logger.info(
            "OllamaLLM ready (base_url=%s, model=%s, embedding_model=%s)",
            base_url, model_name, embedding_model,
        )
    ...
    def generate(self, prompt: str) -> str:
        ...
        for attempt in range(1, self._max_retries + 2):
            ...
            try:
                ...
                last_exc = exc
                logger.warning("Ollama call failed on attempt %d/%d: %s", attempt, self._max_retries + 1, exc)
