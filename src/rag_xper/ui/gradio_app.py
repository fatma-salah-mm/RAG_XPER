...
def handle_file_upload(file_obj, strategy: str) -> str:
    ...
    try:
        ...
        return (
            f"ℹ️ الملف '{file_name}' مفهرس مسبقاً في قاعدة بيانات "
            f"{settings.vector_store_type.upper()} وجاهز للاستعلام فوراً."
        )
...
def create_ui() -> gr.Blocks:
    with gr.Blocks(title="RAG_XPER - Enterprise Multi-Modal RAG") as demo:
        gr.Markdown(
            ...
            مساعد ذكي متعدد اللغات لمعالجة ملفات PDF، Markdown، والنصوص والصور.
            <br>
            **قاعدة المتجهات:** `{settings.vector_store_type.upper()}` |
            **محرك الـ LLM:** `{settings.gemini_model}`
            """
        )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                ingest_btn.click(fn=handle_file_upload, inputs=[file_input, strategy_input], outputs=[ingest_status])

            with gr.Column(scale=2):
                gr.Markdown("### 💬 المحادثة والاستعلام الذكي")
                chatbot = gr.Chatbot(label="سجل الأسئلة والأجوبة", height=450)
                with gr.Row():
                    query_input = gr.Textbox(label="اكتبي سؤالك هنا...", scale=4, lines=1)
                    ask_btn = gr.Button("إرسال ✉️", variant="primary", scale=1)
                with gr.Accordion("🧠 Chain-of-Thought Reasoning", open=False):
                    reasoning_box = gr.Textbox(label="خطوات التفكير", lines=4, interactive=False)
...
def run_ui() -> None:
    """Console script launcher: rag-xper-ui"""
    demo = create_ui()
    demo.launch(server_name="0.0.0.0", server_port=7861, share=False)  # nosec B104
